...
public class CaptureProperties {
    ...
    /**
     * 手动「把当前画面存到待标注」另存时的去重阈值（百分比，0~100，默认 1）。执行模式把当前
     * 识别画面另存为 capture/ 原始截图前，与同一「去重基准」（capture/ + classify/ 全部 PNG）
     * 逐张做平均像素差异比对：仅当与每一张的差异都 ≥ 本值才允许另存；与任一差异低于阈值即拒绝
     * 保存并提示「和哪张重复」。手动另存由人眼确认过才触发，比自动截图更宽松（默认 1% &lt; 自动 3%），
     * 只拦几乎完全一样的重复画面。
     *
     * <p>0 或负数 = 关闭手动另存的去重检查，每次都按原逻辑保存。</p>
     *
     * <p>每次启动的历史重复清理按本值与 {@link #diffThresholdPercent} 中较大者执行
     * （默认 max(3, 1) = 3%），即与自动截图口径一致：差异须大于该值才保留。</p>
     */
