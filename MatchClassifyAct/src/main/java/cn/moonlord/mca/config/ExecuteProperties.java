...
public class ExecuteProperties {
     ...
     * 点击动作且有坐标的分类另有 12 张点击区交集图 click8/32-same100/90/80/70/60/50——
     * 上述对照图全部参与识别比对，逐张同尺度比对得各图「不匹配点占比」后按五族加权：
     * A 全图交集 12 张（权 50）+ B 多数 6 张（权 15）+ C 均值 6 张（权 10）+
     * D 去重均值 6 张（权 10）+ E 点击区交集 12 张（权 15）——每族先对族内各图等权平均，
     * 分类差异度 = (50A+15B+10C+10D+15E)/W（W = 适用族的权重之和：
     * 点击动作分类 42 张齐全 W=100，无点击坐标分类 30 张无 E、W=85；
     * 无有效像素的图（独有区图无独有点等）按 0 完全匹配计、照常参与）。
     ...
     * <b>交集/多数型对照图</b>（交集六档 same100/90/80/70/60/50、major、major8、major32 及各自的
     * -unique 独有区图，另有点击动作分类的 click8/32-same100/90/80/70/60/50 十二张点击区交集图）
     * 固定按「逐像素完全一致」判据
