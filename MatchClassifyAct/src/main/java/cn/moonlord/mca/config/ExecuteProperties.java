...
public class ExecuteProperties {
    ...
    /**
     * 识别阈值（差异度百分比，0~100）。当前画面与每个分类的「汇总分析产物」（summary/ 下 14 张对照图，
     * 7 张基础图 same / max / avg / maj8 / avg8 / maj32 / avg32 及其各自的 -unique 独有区图）
     * 逐张同尺度比对，得到各图各自的「不匹配点占比」后，
     * 将各占比按均方根（RMS）聚合为分类差异度 = √(Σ占比²/14)，固定按 14 张图归一
     * （等价于把 14 维差值向量的长度按维数归一，任一张图
     * 差得远都会抬高总分、不被其余接近的图稀释；RMS ≥ 算术平均，同阈值下判定会比算术平均略严格，
     * 画面实测若常误报未识别可适当放宽本阈值）。不同分类按各自的差异度比较，取最小者为最近似结果。
     * 仅当该差异度 ≤ 本阈值时才判定为「已识别」，
    ...
    /**
     * 判定单个对应像素点是否「匹配」的逐通道色差阈值（0~255，默认 255/3 = 85）。
     * 分别取两个像素 R、G、B 三通道差值的绝对值，只有三个通道的差值都 < 本阈值
     * （即都小于 255/3）才判为「匹配」点；任一通道差 ≥ 本阈值即判为「不匹配」点。
