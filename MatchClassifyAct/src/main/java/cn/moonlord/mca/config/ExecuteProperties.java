...
public class ExecuteProperties {
     ...
     * 7 张核心基础图 same90 / max / avg / major8 / avg8 / major32 / avg32 及其各自的 -unique 独有区图，
     * 点击动作且有坐标的分类另有 click8-same / click32-same 两张点击区交集图；
     * 交集图 same80/same70/same60 为仅目检展示档、不参与识别）逐张
     * 同尺度比对得各图「不匹配点占比」后，按加权平均聚合为分类差异度
     * = (独有交集图占比×50 + 交集图占比×30 + 其余可判成员占比的平均×20) / 100
     * （交集 / 独有交集锁定样本的公共稳定区与独占核心区，权重最高；
     * 「其余图」成员为块图与独有区图，点击动作分类另含两张点击区交集图，缺失维不参与平均）。
     ...
     * <b>交集/多数型对照图</b>（same90、major、major8、major32 及各自的 -unique 独有区图，另有
     * 点击动作分类的 click8-same、click32-same 点击区交集图）固定按「逐像素完全一致」判据
