...
public class ExecuteProperties {
     ...
     * 14 张基础合成图 = 交集五档 same90/80/70/60/50 与 max / avg / dedup-avg / major8 / avg8 /
     * dedup-avg8 / major32 / avg32 / dedup-avg32（去重均值 = 每像素 / 块先把样本出现过的颜色去重
     * 再等权平均，避免抽样不均把均值拉偏），每张基础图各带一张 -unique 独有区图（共 14 张）；
     * 点击动作且有坐标的分类另有 10 张点击区交集图 click8/32-same90/80/70/60/50——
     * 上述对照图全部参与识别比对，逐张同尺度比对得各图「不匹配点占比」后按五族加权：
     * A 全图交集 10 张（权 50）+ B 多数 6 张（权 15）+ C 均值 6 张（权 10）+
     * D 去重均值 6 张（权 10）+ E 点击区交集 10 张（权 15）——每族先对族内各图等权平均，
     * 分类差异度 = (50A+15B+10C+10D+15E)/W（W = 适用族的权重之和：
     * 点击动作分类 38 张齐全 W=100，无点击坐标分类 28 张无 E、W=85；
     ...
     * <b>交集/多数型对照图</b>（交集五档 same90/80/70/60/50、major、major8、major32 及各自的
     * -unique 独有区图，另有点击动作分类的 click8/32-same90/80/70/60/50 十张点击区交集图）
     * 固定按「逐像素完全一致」判据
