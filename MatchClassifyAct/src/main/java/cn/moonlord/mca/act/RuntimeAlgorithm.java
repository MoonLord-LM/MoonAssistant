 ...
 *
 * <p>算法调优每跑完一轮就把它生成到 {@code runtime/}：{@code algorithm.json} 是本类的 JSON 形态，
 * 同目录下按分类分子目录存放该算法<b>生效特征</b>的对照图产物（每个生效特征 kind 一张）。
 * 执行模式与「未标注」的单图智能推荐<b>只认这份落地物</b>（不再直接读 summary/ 下的对照图），
...
public record RuntimeAlgorithm(String algoId, String algoName, double accuracy, double tieRate, double score,
     ...
     * @param dir      相对 {@code runtime/} 的子目录名（= 汇总分析里的分类目录名）
    ...
    public record State(String state, String dir, String action, Integer attnLeft, Integer attnTop,
        ...
        /** 某个特征产物在运行时的绝对路径（{@code root} = runtime/ 目录）；该分类没有此特征产物时返回 null。 */
     ...
     *
     * <p>落地与识别都只用生效特征：{@code runtime/&lt;分类&gt;/} 每个分类只复制这几个 kind 的产物、
