...
| `summary/<分类标注>/` | **汇总分析产物**：对照图（15 张基础合成图 = 交集六档 same100/90/80/70/60/50（100% = 样本像素完全一致，其余覆盖 >90%/80%/70%/60%/50%）与多数/均值/去重均值/8·32 块图族；每张基础图各带 1 张 `-unique` 独有区图，共 15 张——基础图与独有区图**全部参与识别比对**；各分类以统一关注点或点击点坐标（点击分类=点击点 / 无动作分类=画面关注区域，默认屏幕中心）为中心另含 12 张点击区交集图 = 1/8、1/32 方框 × 交集六档，同样全部参与比对）+ `info.json`，仅供人工目检 / 执行识别，可随时整目录删除后重算 |
...
- **匹配动作** = 无动作 / 鼠标点击；两类都要在图上点一个**关注点**（鼠标点击=红色点击位置，无动作=绿色画面关注区域，未点选默认取屏幕中心）。每个分类只有一份定义：**首次**打标时确定，之后同分类加样本直接填分类保存即可，坐标自动沿用；在已标注图上改动作/坐标 = **重定义该分类**（全组同步）。唯一动作只约束**仍在使用（有样本）**的分类；样本删光后中心表残留的空定义视为全新分类，下次首标会直接重新定义，不会挡你新建。
...
进入「汇总分析」段会自动异步分析每个「样本 ≥1 张且尚未分析」的分组；样本有变也会自动标记待重算。产物 = **对照图**（15 张基础合成图 = 交集图分六档：100% 档 = 全部样本像素一致（最严格），其余按覆盖率阈值 >90%/80%/70%/60%/50% 分档，再加多数/均值/去重均值/8·32 块图族；每张基础图各带 1 张对应的 `-unique` 独有区图，共 15 张；各分类以统一关注点或点击点坐标（点击分类=点击点 / 无动作分类=画面关注区域，默认屏幕中心）为中心另有 12 张点击区交集图 = 1/8、1/32 方框 × 交集六档）+ `info.json`（样本数 / 覆盖率 / 统一关注点坐标等），**供人工目检，并作为执行模式实时识别 / 智能分析建议的比对源——只读使用，不自动改动任何标注**（基础图、独有区图与点击区交集图全部参与识别：每个分类 15 + 15 + 12 = 42 张；历史无动作分类旧产物只有 30 张，后台重算后补齐）：
...
| `click8-same100.png` `click32-same100.png`（点击区交集图 100% 档，各分类都生成：统一关注点=点击位置 / 无动作=画面关注区域） | 以本分类**统一关注点或点击点坐标为中心**、整幅长宽各 1/8 与 1/32 的方框交集小图，框内像素须**全部样本完全一致**才保留该真实色——最严格地只留下「要点位置恒定不变」的画面（框几何、越界透明、无 `-unique` 等规则与下行 90% 档一致），**同样参与识别比对** |
| `click8-same90.png` `click32-same90.png`（点击区交集图 90% 档，各分类都生成：统一关注点=点击位置 / 无动作=画面关注区域） | 以本分类**统一关注点或点击点坐标为中心**、整幅长宽各 1/8 与 1/32 的方框交集小图（1280×720 → 160×90 / 40×22），框内逐像素做与交集图 90% 档相同的「覆盖率 >90%」判定，聚焦关注位置（点击分类=要点击的位置）。**框中心固定、不向画幅内收敛**：越出画幅边缘的部分在产物里按透明像素保存，识别时画面按同一坐标裁框、越界处同样填透明——两侧逐点口径一致，越界点不参与统计；无 `-unique` 版、不参与独有区互比 |
| `click8-same80/70/60/50.png` `click32-same80/70/60/50.png`（点击区交集图低档，各分类都生成：统一关注点=点击位置 / 无动作=画面关注区域） | 同上、把达标阈值放宽到 >80% / >70% / >60% / >50% 的框内交集小图（框几何、越界透明、无 `-unique` 等规则与 90% 档一致），**同样参与识别比对**；汇总面板按产物是否齐全（`hasClick` / `hasClickLow`）展示 |
| 15 张 `-unique` 独有区图（`same100-unique` / `same90-unique` / `same80-unique` / `same70-unique` / `same60-unique` / `same50-unique` / `major-unique` / `avg-unique` / `dedup-avg-unique` / `major8-unique` / `avg8-unique` / `dedup-avg8-unique` / `major32-unique` / `avg32-unique` / `dedup-avg32-unique`，全部参与识别比对） | 每张对应一张基础图（交集六档 / 多数 / 均值 / 去重均值 / 8·32 块图族）：在**该 kind 基础图**上去掉「**其它分类同 kind 基础图**同位置同色」的像素，剩本分类独有区域，用来拉开相近分类的差距。均为跨分类产物：**要等全部分组的基础图都生成完才开始算**；**该分类适用的对照图必须齐全**（每分类 15 基础 + 15 -unique + 12 点击区交集图 = 42 张）才参与执行识别——基础图看整幅画面、独有区图只盯本分类独占区、点击区交集图盯要点的位置，互补拉开相近分类的差距。接口 kind 与文件名一致（块图无短码，kind 即 `major8/avg8/dedup-avg8/major32/avg32/dedup-avg32`），仅多数图 kind=`max` 对应磁盘名 `major` |

已分析分组在列表中按交集图（90% 覆盖率档）覆盖率**从低到高**排列：覆盖率越低 = 采到了该状态不同时刻的真实差异（样本更充分），排在前面；覆盖率越高 = 多为同一画面反复截取（采样不足），排在后面提示补采不同状态的画面。未分析 / 待重算分组排在其后（其中样本多的靠前）。单击任一图放大查看，Esc 还原。
...
- **识别口径**：把当前帧与 `summary/` 里各分组适用的对照图（15 张基础合成图 = 交集六档 same100/90/80/70/60/50（100% = 全部样本像素一致）与多数/均值/去重均值/8·32 块图族 + 15 张对应的 `-unique` 独有区图，共 30 张；各分类另含 12 张点击区交集图（以统一关注点或点击点坐标为中心），共 42 张，需按分类齐全才参与）逐张比对。逐点判据按「代表色来源」分两套：**交集/多数/点击区类**（交集六档 same100/90/80/70/60/50 / 多数图 major / 8×8·32×32 多数块图 major8·major32 / 点击区交集图 click8/32-same100/90/80/70/60/50，及各自 -unique（点击区图无 -unique），共 30 张）颜色是样本真实像素，要求**逐像素完全一致**（R/G/B 三通道差都为 0 才判「匹配」，画面同状态因 resize 逐像素对齐应能精确重现；点击区交集图在画面按同一分类关注点或点击点坐标裁方框后比较）；**均值类**（均值图 avg / 去重均值图 dedup-avg / 8×8·32×32 均值块图 avg8·avg32 / 去重均值块图 dedup-avg8·dedup-avg32，及各自 -unique，共 12 张）颜色是样本平均 / 去重平均色（去重平均 = 每像素 / 块内先把样本出现过的颜色去重再等权平均；真实画面不会恰好等于平均色），用逐通道容差（三通道差都 ≤ `execute.rgb-dist-threshold`（默认 255/3=85）判「匹配」，任一通道 > 它判「不匹配点」）。分类差异度 = 按 A~E 五族加权 (50A+15B+10C+10D+15E)/W——A 全图交集 12 张（same100→50 各带 -unique，权 50）、B 多数族 6 张（max/major8/major32 各带 -unique，权 15）、C 均值族 6 张（avg/avg8/avg32 各带 -unique，权 10）、D 去重均值族 6 张（dedup-avg/8/32 各带 -unique，权 10）、E 点击区交集 12 张（click8/32 × 六档，权 15，各分类按关注点或点击点坐标生成）：每族先把族内各图「不匹配点占比」等权平均；W = 适用族的权重之和（点击分类 42 张五族齐全 W=100，无点击坐标分类 30 张无 E、W=85）；各图照常全量参与，产物无任何有效像素的空图（独有区图无独有点 / 基础图公共区全空）没有可判别的点、无法做区分，判完全不匹配、按不匹配占比满值计入；识别以最近似分组为准、**不设识别阈值门槛**（差异度仅作相近程度展示，不再区分「已识别/未识别」）。
...
| `execute.rgb-dist-threshold` | `85`（255/3） | **均值型对照图**（avg/dedup-avg/avg8/dedup-avg8/avg32/dedup-avg32 及各自 -unique）判不匹配的逐通道色差上限：R/G/B 任一通道差 > 它判「不匹配」，三通道都 ≤ 它才匹配；交集/多数/点击区型（交集六档 same100/90/80/70/60/50、major/major8/major32 及各自 -unique、点击区交集图 click8/32-same100/90/80/70/60/50）固定「逐像素完全一致」，不受它影响 |
