...
| `summary/<分类标注>/` | **汇总分析产物**：对照图（10 张基础合成图 = 交集 90 主档 + 80/70/60 展示档 + 多数/均值/8·32 块图；主档交集与其余 6 张核心基础图各带 1 张 `-unique` 独有区图，共 7 张；交集 80/70/60 展示档仅目检、不参与识别；点击动作且坐标有效的分类另含 2 张点击区交集图）+ `info.json`，仅供人工目检 / 执行识别，可随时整目录删除后重算 |
...
进入「汇总分析」段会自动异步分析每个「样本 ≥1 张且尚未分析」的分组；样本有变也会自动标记待重算。产物 = **对照图**（10 张基础合成图 = 交集图按覆盖率阈值分 90 主档 + 80/70/60 展示档，再加多数/均值/8·32 块图；主档交集与其余 6 张核心基础图各带 1 张对应的 `-unique` 独有区图，共 7 张；匹配动作为鼠标点击且坐标有效的分类另有 2 张点击区交集图）+ `info.json`（样本数 / 覆盖率 / 公共点击坐标等），**供人工目检，并作为执行模式实时识别 / 智能分析建议的比对源——只读使用，不自动改动任何标注**（交集 80/70/60 展示档仅目检，不参与识别）：

| 产物 | 含义 |
|------|------|
| `same90.png` 交集图（90% 主档） | 每像素统计样本中主流颜色及其出现占比，覆盖率 ≥90%（达标样本数向上取整）则取该主流色、不足则透明 = 样本间公共（稳定）区；参与识别比对与独有区互比 |
| `same80.png` / `same70.png` / `same60.png` 交集图展示档 | 合成方式同上、仅把达标阈值放宽到 ≥80% / ≥70% / ≥60% 的公共画面，用于目检更低覆盖率下的稳定区。**仅人工目检**：不参与识别比对、无 `-unique` 版、不参与独有区互比 |
| `major.png` 多数图 / `avg.png` 均值图 | 每像素取出现最多的颜色 / 全部样本 RGB 平均 |
| `major8.png` `avg8.png` `major32.png` `avg32.png` | 8×8、32×32 块内「多数色 / 均值色」的降采样合成 |
| `click8-same.png` `click32-same.png`（点击区交集图，**仅点击动作且坐标有效的分类**） | 以本分类**统一点击坐标为中心**、整幅长宽各 1/8 与 1/32 的方框交集小图（1280×720 → 160×90 / 40×22），框内逐像素做与交集图相同的「覆盖率 ≥90%（主档）判据」判定，聚焦要点击的位置。**框中心固定、不向画幅内收敛**：越出画幅边缘的部分在产物里按透明像素保存，识别时画面按同一坐标裁框、越界处同样填透明——两侧逐点口径一致，越界点不参与统计；无 `-unique` 版、不参与独有区互比 |
| 7 张 `-unique` 独有区图（`same90-unique` / `major-unique` / `avg-unique` / `major8-unique` / `avg8-unique` / `major32-unique` / `avg32-unique`；交集 80/70/60 展示档无独有区图） | 每张对应一种核心基础图（主档交集 / 多数 / 均值 / 8·32 块图）：在**该 kind 基础图**上去掉「**其它分类同 kind 基础图**同位置同色」的像素，剩本分类独有区域，用来观察相近分类差在哪。均为跨分类产物：**要等全部分组的 7 张核心基础图都生成完才开始算**；**该分类适用的对照图必须齐全**（无点击坐标分类 14 张；点击动作分类 14 + 2 点击区交集图 = 16 张；交集 80/70/60 展示档不计入）才参与执行识别——基础图看整幅画面、独有区图只盯本分类独占区、点击区交集图盯要点的位置，互补拉开相近分类的差距。接口 kind 与文件名一致（块图无短码，kind 即 `major8/avg8/major32/avg32`），仅多数图 kind=`max` 对应磁盘名 `major` |

已分析分组在列表中按交集图（90% 主档）覆盖率**从低到高**排列：覆盖率越低 = 采到了该状态不同时刻的真实差异（样本更充分），排在前面；覆盖率越高 = 多为同一画面反复截取（采样不足），排在后面提示补采不同状态的画面。未分析 / 待重算分组排在其后（其中样本多的靠前）。单击任一图放大查看，Esc 还原。
...
- **识别口径**：把当前帧与 `summary/` 里各分组适用的对照图（7 张核心基础 = 交集 90 主档 / 多数 / 均值 / 8·32 块图 + 7 张 -unique 独有区图；交集 80/70/60 展示档仅目检不参与比对；点击动作且坐标有效的分类另含 2 张点击区交集图，需按分类齐全才参与）逐张比对。逐点判据按「代表色来源」分两套：**交集/多数/点击区类**（交集图 same90 / 多数图 major / 8×8·32×32 多数块图 major8·major32 / 点击区交集图 click8-same·click32-same，及各自 -unique（点击区图无 -unique），共 10 张）颜色是样本真实像素，要求**逐像素完全一致**（R/G/B 三通道差都为 0 才判「匹配」，画面同状态因 resize 逐像素对齐应能精确重现；点击区交集图在画面按同一分类点击坐标裁方框后比较）；**均值类**（均值图 avg / 8×8·32×32 均值块图 avg8·avg32，及各自 -unique，共 6 张）颜色是样本平均色（真实画面不会恰好等于它），用逐通道容差（三通道差都 ≤ `execute.rgb-dist-threshold`（默认 255/3=85）判「匹配」，任一通道 > 它判「不匹配点」）。分类差异度 = 各图「不匹配点占比」的**加权平均** = **(独有交集图×50 + 交集图×30 + 其余可判成员平均×20) ÷ 100**（交集/独有交集锁定样本的公共稳定区与独占核心区，权重最高；「其余成员」= 块图 + 独有区图，点击动作分类另含 2 张点击区交集图，缺失维度不参与平均，合计只占 20%，个别维明显差异不会被过度放大）；识别以最近似分组为准、**不设识别阈值门槛**（差异度仅作相近程度展示，不再区分「已识别/未识别」）。
...
| `execute.rgb-dist-threshold` | `85`（255/3） | **均值型对照图**（avg/avg8/avg32 及各自 -unique）判不匹配的逐通道色差上限：R/G/B 任一通道差 > 它判「不匹配」，三通道都 ≤ 它才匹配；交集/多数/点击区型（same90/major/major8/major32/click8-same/click32-same 及各自 -unique；交集 80/70/60 展示档不参与比对）固定「逐像素完全一致」，不受它影响 |
...
- 汇总分析：`GET /api/annotate/think/groups`、`POST /api/annotate/think/analyze`、`POST …/rebuild`（一键重建全部产物）、`GET …/task/{id}`、`GET …/img/{kind}?dir={dirB64}`（`kind = 图之一：same90|same90-unique|same80|same70|same60|max|max-unique|avg|avg-unique|major8|major8-unique|avg8|avg8-unique|major32|major32-unique|avg32|avg32-unique|click8-same|click32-same`，`dir` 为分类目录名 UTF-8 → Base64）。
