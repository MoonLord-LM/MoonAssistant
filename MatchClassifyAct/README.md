...
| `summary/<分类标注>/` | **汇总分析产物**：对照图（15 张基础合成图 = 交集六档 same100/90/80/70/60/50（100% = 样本像素完全一致，其余覆盖 >90%/80%/70%/60%/50%）与多数/均值/去重均值/8·32 块图族；每张基础图各带 1 张 `-unique` 独有区图，共 15 张——基础图与独有区图**全部参与识别比对**；各分类以统一关注点坐标（点击=点击位置 / 无动作=画面关注区域，默认屏幕中心）为中心另含 12 张点击区交集图 = 1/8、1/32 方框 × 交集六档，同样全部参与比对）+ `info.json`，仅供人工目检 / 执行识别，可随时整目录删除后重算 |
...
进入「汇总分析」段会自动异步分析每个「样本 ≥1 张且尚未分析」的分组；样本有变也会自动标记待重算。产物 = **对照图**（15 张基础合成图 = 交集图分六档：100% 档 = 全部样本像素一致（最严格），其余按覆盖率阈值 >90%/80%/70%/60%/50% 分档，再加多数/均值/去重均值/8·32 块图族；每张基础图各带 1 张对应的 `-unique` 独有区图，共 15 张；各分类以统一关注点坐标（点击=点击位置 / 无动作=画面关注区域，默认屏幕中心）为中心另有 12 张点击区交集图 = 1/8、1/32 方框 × 交集六档）+ `info.json`（样本数 / 覆盖率 / 统一关注点坐标等），**供人工目检，并作为执行模式实时识别 / 智能分析建议的比对源——只读使用，不自动改动任何标注**（基础图、独有区图与点击区交集图全部参与识别：每个分类 15 + 15 + 12 = 42 张；历史无动作分类旧产物只有 30 张，后台重算后补齐）：
...
| `click8-same100.png` `click32-same100.png`（点击区交集图 100% 档，各分类都生成：统一关注点=点击位置 / 无动作=画面关注区域） | 以本分类**统一关注点坐标为中心**、整幅长宽各 1/8 与 1/32 的方框交集小图，框内像素须**全部样本完全一致**才保留该真实色——最严格地只留下「要点位置恒定不变」的画面（框几何、越界透明、无 `-unique` 等规则与下行 90% 档一致），**同样参与识别比对** |
| `click8-same90.png` `click32-same90.png`（点击区交集图 90% 档，各分类都生成：统一关注点=点击位置 / 无动作=画面关注区域） | 以本分类**统一关注点坐标为中心**、整幅长宽各 1/8 与 1/32 的方框交集小图（1280×720 → 160×90 / 40×22），框内逐像素做与交集图 90% 档相同的「覆盖率 >90%」判定，聚焦关注位置（点击分类=要点击的位置）。**框中心固定、不向画幅内收敛**：越出画幅边缘的部分在产物里按透明像素保存，识别时画面按同一坐标裁框、越界处同样填透明——两侧逐点口径一致，越界点不参与统计；无 `-unique` 版、不参与独有区互比 |
